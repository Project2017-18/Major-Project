...
class ParticleSwarm(Algorithm):
    ...
    def initialize(self):
        ...
        row, col=np.array(self.particles[0].variables).shape
##        self.velocities = [[0.0]*self.problem.nvars for _ in range(self.swarm_size)]
        self.velocities = [[[0.0]*(col)]*self.problem.nvars for _ in range(self.swarm_size)]
    ...
    def _update_velocities(self):
        for i in range(self.swarm_size):
            ...
            row, col=np.array(particle).shape
...
##                        C2*r2*(leader[j] - particle[j])
            ...
            for j in range(self.problem.nvars):
                for k in range(col):
                    ...
                    self.velocities[i][j][k] = W * self.velocities[i][j][k] + \
                            C1*r1*(local_best[j][k] - particle[j][k]) + \
                            C2*r2*(leader[j][k] - particle[j][k])
                    self.velocities[i][j][k]=sigmoid(self.velocities[i][j][k])
    ...
    def _update_positions(self):
        for i in range(self.swarm_size):
            ...
            for j in range(self.problem.nvars):
                row, col=np.array(self.particles[i].variables).shape
                for k in range(col):
                     if random.uniform(0.0, 1.0) < self.velocities[i][j][k]:
                          offspring.variables[j][k]=not offspring.variables[j][k]
